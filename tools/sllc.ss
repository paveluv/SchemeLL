...
;; target setup and optimization run lazily: only the modes that
;; codegen need a backend, so e.g. --render-llvm-ir works for programs
;; declaring triples this libLLVM cannot even target
(define get-tm
  (let ([tm #f])
    (lambda ()
      (unless tm
...
;; an output path may never coincide with the input (finding: --exe on
;; an extensionless input silently replaced the source with the binary)
(define (checked-out ext)
  (let ([path (default-out ext)])
    (when (string=? path in-path)
...
(define (check-exe-supported! obj)
  (unless exe-osabi
    (error 'sllc
      ...
      (machine-type)))
  ;; a declared cross-OS triple would yield a binary branded for THIS
  ;; host but built against another kernel's ABI
  (when (and declared-triple host-os)
    (let ([n (string-length declared-triple)]
          [m (string-length host-os)])
      (unless (let loop ([i 0])
                (cond
                  [(> (+ i m) n) #f]
                  [(string=? (substring declared-triple i (+ i m)) host-os)
                   #t]
                  [else (loop (+ i 1))]))
        (error 'sllc
          "--exe runs on this host's kernel; the program declares another OS -- emit a .o instead"
          declared-triple host-os))))
...
(define (emit-executable obj path)
  ...
  (let* ([secs (sections obj)]
    ...
    ;; any loadable data (ALLOC + PROGBITS/NOBITS, size > 0) other than
    ;; .text would be silently absent from the executable; refuse by
    ;; FLAGS, which also covers .rodata.cst8-style suffixed names and
    ;; .bss (NOBITS). .eh_frame (X86_64_UNWIND) is droppable: nothing
    ;; unwinds in a freestanding binary.
    (for-each
      (lambda (s)
        (when (and (not (eq? s text))
                   (memv (s-type s) '(1 8))          ; PROGBITS, NOBITS
                   (positive? (bitwise-and (s-flags s) 2))   ; SHF_ALLOC
                   (> (s-size s) 0))
          (error 'sllc "--exe handles only code; found a loadable data section"
                 (s-name s))))
      secs)
    ;; find _start's offset inside .text via the symbol table
    (let* ([symtab (or (find (lambda (s) (= (s-type s) 2)) secs)  ; SYMTAB
           ...
           [start-off
            (let loop ([off (s-off symtab)])
              (if (>= off (+ (s-off symtab) (s-size symtab)))
                  (error 'sllc "--exe requires a @_start function")
                  (let ([nm (cstr obj (+ (s-off strtab)
                                         (bv-u32 obj off)))]
                        [shndx (bv-u16 obj (+ off 6))])
                    (if (and (string=? nm "_start") (= shndx text-index))
                        (bv-u64 obj (+ off 8))
                        (loop (+ off 24))))))]
           [text-bytes (let ([b (make-bytevector (s-size text))])
                         (bytevector-copy! obj (s-off text) b 0
                                           (s-size text))
                         b)]
           [base #x400000]
           ;; place .text honoring its alignment requirement (the
           ;; assembler laid it out against a 2^n-aligned base; 0x78
           ;; alone is 8 mod 16 and misaligns aligned constants/jump
           ;; tables inside the section)
           [text-align (max 1 (s-align text))]
           [text-off (let ([h #x78])        ; ehdr (64) + one phdr (56)
                       (* (div (+ h text-align -1) text-align)
                          text-align))]
      ...
      (bytevector-copy! text-bytes 0 exe text-off
...
(case mode
  [(render)
   ...
   ;; and verified the program. --opt has nothing to act on here.
   (when opt-level
     (die "--opt has no effect with --render-llvm-ir (it prints the program as written)"))
   (emit-text (render:sll->ll prog))]
  [(canonical)
   (get-tm)   ; configure the module (and apply --opt) before printing
   (emit-text (ir:module->string m))]
  [(object)
   (let ([path (checked-out ".o")])
     (target:emit-object-file (get-tm) m path)
     (printf "wrote ~a~%" path))]
  [(asm)
   (let ([path (checked-out ".s")])
     (target:emit-assembly-file (get-tm) m path)
     (printf "wrote ~a~%" path))]
  [(run)
   ;; --run executes @main (hosted semantics: exit with its return
   ;; value; the classic (i32 @main (i32 %argc) (ptr %argv)) form is
   ;; called with argc 0, argv NULL) or, when there is none, @_start
   ;; (freestanding semantics: the program exits ITSELF, usually via a
   ;; raw exit syscall that ends this process on the spot -- flush
   ;; first).
   ...
   (let* ([fn-params
     ...
     (unless (or main-params start-params)
       ...
       (cond
         ...
         [else
          (let ([start (jit:function j "_start")])
            ...
            (exit 0))])))]
  [(exe)
   (emit-executable (target:emit-object-bytevector (get-tm) m)
                    (checked-out ""))])
