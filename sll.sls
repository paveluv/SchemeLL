...
;;; (create all blocks, then emit their instructions), so branches and phi
;;; incoming may reference blocks defined later. phi is the only permitted
;;; forward reference to a *value*; everything else must be defined textually
;;; before use.
[library
 (sll)
 [export
 ...
 (define (error msg . irritants) (apply base:error 'sll:build msg irritants))
 ...
 [define
  (strip-sigil x)
  (let ((s (symbol->string x))) (substring s 1 (string-length s)))]

 ;; All-digit names (%0, %42) are positional/anonymous, as in textual IR where
 ;; digits are slot numbers, not names: sll binds them in its own environment
 ;; but leaves the LLVM value unnamed, so LLVM's printer reproduces the
 ;; numbering itself.
 [define
  (anonymous-name? s)
  [and
 ...
 [define
  (llvm-name x)                 ; %sym -> the name to give LLVM ("" = unnamed)
  (let ((s (strip-sigil x))) (if (anonymous-name? s) "" s))]

 ;; Build a %/@ name from pieces -- the generator's replacement for the
 ;; (string->symbol (format ...)) dance: (name '%p 1) => %p1, (name '@h op) =>
 ;; @h3. The first piece carries the sigil; pieces are symbols, strings, or
 ;; exact integers; the result is an interned symbol, so name equality stays
 ;; eq?. A CONSTRUCTED all-digit name raises: it would fall under the anonymity
 ;; rule (positional, printer-numbered), which is almost always an accident when
 ;; a name is being built -- spell such names literally if they are really
 ;; intended.
 [define
  (name piece0 . pieces)
  [define
   ...
   [cond
    ...
    [else
     [error
      "name piece must be a symbol, string or exact integer"
      p
      (cons piece0 pieces)]]]]
  [let
   ((head (piece->string piece0)))
   [unless
    (and (> (string-length head) 0) (memv (string-ref head 0) '(#\% #\@)))
    [error
     "the first name piece must carry the % or @ sigil"
     piece0
     (cons piece0 pieces)]]
   [let
    ((s (apply string-append head (map piece->string pieces))))
    [when
     (= (string-length s) 1)
     (error "constructed name is empty" (cons piece0 pieces))]
    [when
     (anonymous-name? (substring s 1 (string-length s)))
     [error
      "constructed name is all digits and would be anonymous under the anonymity rule; spell it literally if that is intended"
      s
      (cons piece0 pieces)]]
    (string->symbol s)]]]
 ...
 [define
  (int-bits t)                  ; i1, i8, ..., iN -> N; anything else -> #f
  [let
   ((s (symbol->string t)))
   [and
    (> (string-length s) 1)
 ...
 ;; strip an optional trailing `variadic` marker (never spelled `...`, which
 ;; would collide with ellipsis in nanopass/syntax-rules patterns)
 [define
  (split-variadic lst)
  [if
   (and (pair? lst) (eq? (car (last-pair lst)) 'variadic))
   (values (reverse (cdr (reverse lst))) #t)
   (values lst #f)]]

 ;; iN, float, double, ptr, void; (ptr (addrspace N)); (array N TY); (vector N
 ;; TY); (struct TY ...); (fn RET ARG ... variadic?) function types
 [define
  (resolve-type ctx t)
  [cond
   [(symbol? t)
    [case
     ...
     [else
      [if
       ...
       (or (lookup-type ctx (strip-sigil t)) (error "unknown named type" t))
       [let
        ((bits (int-bits t)))
   ...
   [(pair? t)
    [cond
     ...
     ;; lengths are uint64 in LLVM ([0 x T] and beyond-fixnum sizes are both
     ;; legal), so exact integers, not fixnums
     [[and
       (eq? (car t) 'array)
       (= (length t) 3)
       (exact? (cadr t))
     ...
     [(eq? (car t) 'target-ext)
      ...
      [unless
       (and (>= (length t) 2) (string? (cadr t)))
       (error "expected (target-ext \"name\" params...)" t)]
      [let
       loop
       ((ps (cddr t)) (tys '()))
       [if
        (or (null? ps) (integer? (car ps)))
        [begin
         [unless
          (for-all (lambda (x) (and (integer? x) (exact? x) (>= x 0))) ps)
          (error "target-ext int params must trail the type params" t)]
         [ir:target-ext-type
          ctx
          (cadr t)
          (map (lambda (x) (resolve-type ctx x)) (reverse tys))
          ps]]
        (loop (cdr ps) (cons (car ps) tys))]]]
     [(eq? (car t) 'fn)
      [let-values
       (((parts variadic?) (split-variadic (cdr t))))
       [unless
        (pair? parts)
 ...
 ;; element a (type value) pair; disambiguated from a single typed operand group
 ;; by the first element's head never being a type constructor heads that make a
 ;; pair a TYPE form; ptr only as (ptr (addrspace N)) -- (ptr X) with any other
 ;; X is a ptr-typed operand group
 [define
  (type-form? h)
  [and
   (pair? h)
 ...
 [define
  ...
  [and
   (pair? form)
   (for-all (lambda (e) (and (pair? e) (pair? (cdr e)) (null? (cddr e)))) form)
   [or
    (not (= (length form) 2))
    [let
     ((h (car form)))
     (and (pair? h) (not (type-form? h)) (not (local-name? (car h))))]]]]

 ;; metadata operand: (md "string") | (md (element ...)) -> MetadataRef
 [define
  (resolve-md-ref ctx form)
  [unless
   (and (pair? form) (eq? (car form) 'md) (= (length form) 2))
   (error "expected (md \"string\") or (md (element ...))" form)]
  [let
   ((x (cadr form)))
   [cond
    ((string? x) (ir:md-string ctx x))
    ((list? x) (ir:md-node ctx (map (lambda (e) (resolve-md-ref ctx e)) x)))
    (else (error "expected (md \"string\") or (md (element ...))" form))]]]
 ...
 [define
  ...
  [unless
   (and (bundle-form? bf) (>= (length bf) 2) (string? (cadr bf)))
 ...
 [define
  ...
  [cond
   ...
   [(local-name? form)
    [or
     (hashtable-ref (fstate-locals st) form #f)
     ;; not defined yet: legal when the defining block only appears textually
     ;; later (dominance is what matters, and LLVM's own printer emits such IR)
     ;; -- create a placeholder to patch at end of function. Needs a type;
     ;; untyped positions cannot forward-reference.
     (and ty (forward-placeholder st ty form))
     [error
      "unbound local in an untyped position (cannot forward-reference)"
      form
      (fstate-fname st)]]]
   [(global-name? form)
    [or
     (hashtable-ref (fstate-globals st) form #f)
   ...
   [(and (pair? form) (eq? (car form) 'blockaddress))
    ...
    [let*
     [(fname (cadr form))
      [fn
       [or
        (hashtable-ref (fstate-globals st) fname #f)
        (error "unbound function in blockaddress" form)]]
      [tbl
       [if
        (eq? fname (fstate-fname st))
        (fstate-blocks st)
        [or
         (and (function-blocks) (hashtable-ref (function-blocks) fname #f))
         (error "blockaddress target is not a define" form)]]]
      [bb
       [or
        (hashtable-ref tbl (caddr form) #f)
        (error "unknown label in blockaddress" form)]]]
     (ir:block-address fn bb)]]
   [(memq form '(null zeroinitializer none))
    (unless ty (error "null/zeroinitializer/none needs a type annotation" form))
    (ir:const-null ty)]
   ...
   [(and (pair? form) (eq? (car form) 'md))
    (ir:metadata-value (fstate-ctx st) (resolve-md-ref (fstate-ctx st) form))]
   [[and
     (pair? form)
     [memq
      (car form)
      '[trunc
        ...
        extractelement
        insertelement]]]
    ;; a constant expression in operand position; splat is typed by its context,
    ;; the rest are self-typed
    [resolve-constant
     (fstate-ctx st)
     (fstate-globals st)
     ty
 ...
 ;; a unique, typed, erasable stand-in for a not-yet-defined %name: a
 ;; freeze-of-undef instruction in a scratch block that is deleted after
 ...
 [define
  (fixup-forwards! st)
  [let-values
   (((names phs) (hashtable-entries (fstate-pending st))))
   [vector-for-each
    [lambda
     (name ph)
     [let
    ...
    names
    phs]]
  (when (fstate-scratch st) (ir:delete-block! (fstate-scratch st)))]
 ...
 [define
  (block-by-name st name)
  (unless (local-name? name) (error "invalid label name" name))
  [or
   (hashtable-ref (fstate-blocks st) name #f)
   (error "unknown label" name (fstate-fname st))]]

 [define
  (block-ref st form)           ; a (label %x) branch target
  [unless
   [and
    (label-form? form)
    (pair? (cdr form))
 ...
 [define
  (check-block-group g fname)
  [unless
   (label-form? g)
   [error
    "instruction outside a block (expected (label %name insn ...))"
    g
    fname]]
  [unless
   (and (pair? (cdr g)) (local-name? (cadr g)))
   (error "block label must be a %name" g fname)]
  (when (null? (cddr g)) (error "empty block" (cadr g) fname))
  [unless
   (terminator-form? (car (last-pair g)))
   (error "block does not end in a terminator" (cadr g) fname)]]

 ;; fname -> (label -> block) for every define, filled BEFORE any body or
 ;; initializer is emitted, so blockaddress may cross functions
 (define function-blocks (make-parameter #f))

 [define
  (prepare-blocks! ctx globals item)
  [when
   (eq? (item-kind item) 'define)
   [let-values
    (((retty-form fname params lk ccv full-body) (item-signature item)))
    [let*
     [(f (hashtable-ref globals fname #f))
      [body
       [let
        skip
        ((b full-body))
     ...
     [for-each
      [lambda
       ...
       [let
        ((name (cadr g)))
        (when (hashtable-ref tbl name #f) (error "duplicate label" name fname))
        (hashtable-set! tbl name (ir:append-block ctx f (llvm-name name)))]]
      body]
 ...
 [define
  casts                         ; (op type value to type)
  `[(trunc         . ,ir:build-trunc        )
    (zext          . ,ir:build-zext         )
    (sext          . ,ir:build-sext         )
    (fptrunc       . ,ir:build-fptrunc      )
    (fpext         . ,ir:build-fpext        )
    (fptosi        . ,ir:build-fp->si       )
    (fptoui        . ,ir:build-fp->ui       )
    (sitofp        . ,ir:build-si->fp       )
    (uitofp        . ,ir:build-ui->fp       )
    (ptrtoint      . ,ir:build-ptr->int     )
    (inttoptr      . ,ir:build-int->ptr     )
    (bitcast       . ,ir:build-bitcast      )
    (addrspacecast . ,ir:build-addrspacecast)]]

 ;; LLVMAtomicOrdering; cross-checked against the headers by the coverage tests.
 ;; NotAtomic (0) is the absence of an ordering, not writable.
 [define
  atomic-orderings
  '[(unordered . 1)
    (monotonic . 2)
 ...
 ;; callee of call/invoke/callbr: a function/pointer operand, or inline asm:
 ;; (asm "template" "constraints" flag ...), flags: sideeffect alignstack.
 ...
 [define
  (resolve-callee* st fnty form as)
  [if
   (and (pair? form) (eq? (car form) 'asm))
   [begin
    [unless
     [and
      (>= (length form) 3)
      ...
      (string? (caddr form))
      [for-all
       (lambda (f) (memq f '(sideeffect alignstack inteldialect unwind)))
       (cdddr form)]]
   ...
   ;; the callee slot is ptr-typed: lets undef/null callees and forward
   ;; references through; a call-site (addrspace n) marker types constant
   ;; callees outside the program space
 ...
 ;; call-site function type is then built from the argument groups) or a full
 ;; (fn ...) type -- required for vararg calls, as in IR's `call i32 (ptr, ...)
 ;; @printf(...)`. -> (values fn-type result-type arg-values)
 [define
  (callsite-signature st ctx ty-form groups form)
  [for-each
   [lambda
    (g)
    [unless
     (and (pair? g) (pair? (cdr g)) (null? (cddr g)))
     (error "call argument must be (type value)" g form)]]
   groups]
  [let
   [[avals
     [map
      (lambda (g) (resolve-operand st (resolve-type ctx (car g)) (cadr g)))
      groups]]]
   [if
    (and (pair? ty-form) (eq? (car ty-form) 'fn))
    [let
     ((fnty (resolve-type ctx ty-form)))
     (values fnty (ir:type-return-type fnty) avals)]
    [let
     ((retty (resolve-type ctx ty-form)))
     [values
      [ir:function-type
       retty
       (map (lambda (g) (resolve-type ctx (car g))) groups)]
      retty
      avals]]]]]
 ...
 [define
  (unwind-dest st rest form)
  [cond
   ((equal? rest '(caller)) #f)
   ((and (pair? rest) (null? (cdr rest))) (block-ref st (car rest)))
   [else
    [error
     "expected unwind destination: caller or (label %x)"
     form
     (fstate-fname st)]]]]
 ...
 ;; before the predicate). They are peeled off the front of the argument list,
 ;; validated per opcode, and applied to the built instruction via the C API
 ;; setters (getelementptr takes its no-wrap mask at construction instead).

 ;; LLVMFastMathFlags bits; cross-checked against the installed headers by
 ;; tests/test-coverage.ss
 [define
  fmf-bits
  '[(reassoc  . 1  )
    (nnan     . 2  )
 ...
 [define
  ...
  [let
   ...
   ((r rest) (flags '()))
   [if
    (and (pair? r) (symbol? (car r)) (memq (car r) flag-symbols))
    (loop (cdr r) (cons (car r) flags))
    (values (reverse flags) r)]]]

 [define
  (require-flag-op op ops flag form)
  (unless (memq op ops) (error "flag is not valid for this opcode" flag form))]
     ...
     [fold-left
      [lambda
       ...
       [case
        flag
        [(nsw)
         (require-flag-op op wrap-flag-ops flag form)
         (ir:set-nsw! v)
         mask]
        [(nuw)
         (require-flag-op op wrap-flag-ops flag form)
         ...
         mask]
        [(exact)
         (require-flag-op op exact-flag-ops flag form)
         (ir:set-exact! v)
         mask]
        [(disjoint)
         (require-flag-op op '(or) flag form)
         (ir:set-disjoint! v)
         mask]
        [(nneg)
         (require-flag-op op '(zext uitofp) flag form)
         (ir:set-nneg! v)
         mask]
        [(volatile)
        ...
        [(atomic)               ; ordering applied by the load/store handler
         (require-flag-op op '(load store) flag form)
         mask]
        ((weak) (require-flag-op op '(cmpxchg) flag form) (ir:set-weak! v) mask)
        [(singlethread)
         (require-flag-op op '(load store fence atomicrmw cmpxchg) flag form)
         (ir:set-atomic-single-thread! v)
         mask]
        [(tail)
         (require-flag-op op '(call) flag form)
         (ir:set-tail-call-kind! v 1)
         mask]
        [(musttail)
         (require-flag-op op '(call) flag form)
         (ir:set-tail-call-kind! v 2)
         mask]
        [(notail)
         (require-flag-op op '(call) flag form)
         (ir:set-tail-call-kind! v 3)
         mask]
        [(inbounds nusw)
 ...
 ;; instructions that may (and must) end a block; invoke and catchswitch are
 ;; terminators that also bind a result via =
 [define
  terminator-ops
  '[ret
    br
   ...
   [lambda
    ...
    [cond
     ...
     [[and
       ...
       (null? (cddr a))
       (string? (cadr a))]
      ;; place the global in a named section -- a zero-size constant in
      ;; .llvm_stackmaps names a module's map's start under a symbol of the
      ;; module's own choosing
     ...
     [[and
      ...
      ;; hidden: not exported from the dylib -- a symbol every module of a jit
      ;; defines binds to ITS OWN definition (the per-module stackmap keeper's
      ;; need)
      [ir:set-visibility!
       v
       (case (cadr a) ((default) 0) ((hidden) 1) (else 2))]]
     [[and
       (pair? a)
       (eq? (car a) 'addrspace)
       (pair? (cdr a))
       (null? (cddr a))
       (fixnum? (cadr a))]
      ;; the builder places allocas in the datalayout's A space; the annotation
      ;; is declarative -- verify it landed there
      [unless
       (= (cadr a) (ir:value-address-space v))
       [error
        "alloca address space must match the datalayout's alloca space"
        a
        form]]]
 ...
 [define
  (emit-op st form name)
  [let-values
   (((flags args) (span-flags (cdr form))))
   [let
    ((op (car form)) (b (fstate-builder st)) (ctx (fstate-ctx st)))
    [define
     (arity n shape)
     [unless
      (= (length args) n)
      ...
      [else
       [case
        ...
        [(phi)
         [arity>=
          1
          "(phi type (value %label) ...)"] ; zero-incoming phis are legal
                                           ; parse-level IR
         ;; emit empty; incoming resolves at end of function, when every value
         ;; and label is bound (IR's only forward value ref)
         [let
          ((ph (ir:build-phi b (resolve-type ctx (car args)) name)))
          [fstate-phis-set!
           st
           (cons (list ph (car args) (cdr args) form) (fstate-phis st))]
          ph]]
        [(call)
         ...
         [let*
          ...
          [unless
           (pair? app)
           (error "call expects an application group (callee args...)" form)]
          [let-values
           [[(fnty retty avals)
           ...
           [let*
            [(callee (resolve-callee st fnty (car app) (or callee-as 0)))
             [attr-group?
              (lambda (x) (and (pair? x) (eq? (car x) 'attributes)))]
             ...
             [v
              [if
               ...
               [let
                ((brefs (map (lambda (bf) (resolve-bundle st bf)) bundles)))
                [let
                 ((v (ir:build-call-bundles b fnty callee avals brefs name)))
        ...
        [(callbr)
         ...
         [arity>=
          4
          "(callbr type ((asm ...) args...) bundles... (label %fall) ((label %i) ...))"]
         [let*
          [(ccv (cc-spec (car args) form))
           (args (if ccv (cdr args) args))
           (bundles (filter bundle-form? (cddr args)))
          ...
          [unless
           ...
           [error
            "callbr requires an inline-asm callee (LLVM restriction)"
            form]]
          [unless
           (list? (cadddr args))
           (error "callbr expects a list of indirect (label %x) targets" form)]
          [let-values
           [[(fnty retty avals)
             (callsite-signature st ctx (car args) (cdr app) form)]]
           [let
        ...
        [(landingpad)
         ...
         ;; constant) | (filter type constant)
         (arity>= 1 "(landingpad type cleanup|(catch ty c)|(filter ty c) ...)")
         [let
          [[lp
            [ir:build-landingpad
             b
             (resolve-type ctx (car args))
             (length (cdr args))
             name]]]
          [for-each
           [lambda
            ...
            [cond
             ...
             [(and (pair? c) (memq (car c) '(catch filter)) (= (length c) 3))
              [ir:add-clause!
               lp
               [resolve-constant
                ctx
        ...
        [(catchswitch)
         ...
         [unless
          (list? (cadr args))
          (error "catchswitch expects a list of (label %h) handlers" form)]
         [let*
          [(handlers (cadr args))
           [cs
            [ir:build-catchswitch
             b
             (parent-pad st ctx (car args))
             (unwind-dest st (cddr args) form)
             (length handlers)
             name]]]
          (for-each (lambda (h) (ir:add-handler! cs (block-ref st h))) handlers)
          cs]]
        [(catchpad cleanuppad)
         ...
         [let
          [(parent (parent-pad st ctx (car args)))
           (pargs (map (lambda (g) (resolve-operand st #f g)) (cadr args)))]
          [if
           (eq? op 'catchpad)
        ...
        [(catchret)
         ...
         [ir:build-catchret
          b
          (resolve-operand st (ir:token-type ctx) (car args))
        ...
        [(load)
         ...
         [let-values
          (((ord attrs) (split-ordering (cddr args) form)))
          [let
           [[v
             [ir:build-load
              b
         ...
         [let*
          [(ty (resolve-type ctx (car args)))
           ...
           [v
            [if
             count
             (ir:build-array-alloca b ty (resolve-operand st #f count) name)
        ...
        [(va_arg)
         ...
         [ir:build-va-arg
          b
          (resolve-operand st #f (car args))
          (resolve-type ctx (cadr args))
          name]]
        [(extractelement)
         (arity 2 "(extractelement (vec-type v) (int-type i))")
         [ir:build-extractelement
          b
          (resolve-operand st #f (car args))
          (resolve-operand st #f (cadr args))
          name]]
         ...
         [let
          ...
          [unless
           [and
            ...
            (eq? (car m) 'mask)
            (pair? (cdr m))
            (for-all (lambda (x) (or (fixnum? x) (eq? x 'poison))) (cdr m))]
           (error "shufflevector mask must be (mask int|poison ...)" m form)]
          [let
           ((i32 (resolve-type ctx 'i32)))
           [ir:build-shufflevector
            b
            (resolve-operand st #f (car args))
            (resolve-operand st #f (cadr args))
            [ir:const-vector
             [map
              [lambda
               (i)
               (if (eq? i 'poison) (ir:poison-value i32) (ir:const-int i32 i))]
              (cdr m)]]
            name]]]]
        [(extractvalue)
        ...
        [(atomicrmw)
         ...
         [let
          ((rmw (assq (car args) rmw-ops)))
          (unless rmw (error "unknown atomicrmw operation" (car args) form))
          [let
           [[v
             [ir:build-atomicrmw
              b
        ...
        [(cmpxchg)
         [arity>=
          5
          "(cmpxchg (ptr p) (type cmp) (type new) succ-ord fail-ord (align n)?)"]
         [let
          [[v
            [ir:build-cmpxchg
             b
        ...
        [(switch)
         [arity>=
          3
          "(switch type value (label %else) ((type c) (label %l)) ...)"]
         [let*
          [(ty (resolve-type ctx (car args)))
           (v (resolve-operand st ty (cadr args)))
           (cases (cdddr args))
           [sw
            (ir:build-switch b v (block-ref st (caddr args)) (length cases))]]
          [for-each
           [lambda
            (c)
            [unless
             (and (pair? c) (pair? (cdr c)) (null? (cddr c)))
             (error "switch case must be ((type const) (label %l))" c form)]
            [ir:add-case!
             sw
             (resolve-operand st ty (car c))
             (block-ref st (cadr c))]]
           cases]
          sw]]
        [(indirectbr)
         ...
         [let
          ...
          [for-each
           ...
           (cdr args)]
          ibr]]
        ((unreachable) (arity 0 "(unreachable)") (ir:build-unreachable b))
        [(br)
         [cond
          ...
          [(= (length args) 4)  ; (br i1 %c (label %a) (label %b))
           [let
            ((c (resolve-operand st (resolve-type ctx (car args)) (cadr args))))
            [ir:build-cond-br
             b
             c
             (block-ref st (caddr args))
          ...
          [else
           [error
            "expected (br (label %x)) or (br i1 %c (label %a) (label %b))"
            form
            (fstate-fname st)]]]]
        [(ret)
         [cond
          ...
          [else
           [error
            "expected (ret void) or (ret type value)"
            form
            (fstate-fname st)]]]]
 ...
 [define
  (emit-insn! st form)
  [cond
   ((not (pair? form)) (error "invalid instruction" form (fstate-fname st)))
   [(label-form? form)
    [error
     "blocks do not nest: (label ...) inside a block"
     form
     (fstate-fname st)]]
   [(eq? (car form) '=)
    ;; (= %name (op ...)) or, with instruction-attached metadata, (= %name (op
    ;; ...) (!md "kind")) -- the !kind marker some frontend passes read
    ;; (presence only; empty node)
    [unless
     [and
      (memv (length form) '(3 4))
      (local-name? (cadr form))
      (pair? (caddr form))
      [or
       (= (length form) 3)
       [let
        ...
        [and
         ...
         (string? (cadr m))]]]]
     [error
      "expected (= %name (op ...)) with optional (!md \"kind\")"
      form
      (fstate-fname st)]]
    [let
     ((lhs (cadr form)) (rhs (caddr form)))
     [when
      (memq (car rhs) no-result-ops)
      (error "instruction produces no result to bind" form)]
     [when
      (hashtable-ref (fstate-locals st) lhs #f)
      (error "duplicate local name" lhs (fstate-fname st))]
     [let
      ((v (emit-op st rhs (llvm-name lhs))))
      [when
       (= (length form) 4)
       (ir:set-instruction-metadata! (fstate-ctx st) v (cadr (cadddr form)))]
     ...
     [ir:phi-add-incoming!
      ...
      [map
       [lambda
        (pr)
        [unless
         (and (pair? pr) (pair? (cdr pr)) (null? (cddr pr)))
         (error "phi incoming must be [value %label]" pr form)]
        (cons (resolve-operand st ty (car pr)) (block-by-name st (cadr pr)))]
       pairs]]]]
   (reverse (fstate-phis st))]]

 ;; ---- global variables and constant initializers ---------------------------

 ;; LLVMLinkage; cross-checked against the headers by the coverage tests. Keys
 ;; are the IR keywords.
 [define
  linkages
  '[(external             . 0 )
 ...
 ;; llvm::CallingConv; keys are the IR keywords, i.e. exactly the names LLVM
 ;; 19's printer uses (probed by setting every id 0..120 and reading the print).
 ;; Ids the printer has no name for spell as (cc N) -- and named ones MUST use
 ;; the name, so sll data is canonical both directions (same shape as the
 ;; anonymity rule). ccc (0) is the default and is never written.
 [define
  call-convs
  '[(fastcc                           . 8  )
    (coldcc                           . 9  )
    ...
    (amdgpu_ls                        . 95 )
    (amdgpu_es                        . 96 )
    (aarch64_vector_pcs               . 97 )
    (aarch64_sve_vector_pcs           . 98 )
    (amdgpu_gfx                       . 100)
    (aarch64_sme_preservemost_from_x0 . 102)
    (aarch64_sme_preservemost_from_x2 . 103)
    (amdgpu_cs_chain                  . 104)
    (amdgpu_cs_chain_preserve         . 105)
    (m68k_rtdcc                       . 106)
    (graalcc                          . 107)
    (riscv_vector_cc                  . 110)
    (aarch64_sme_preservemost_from_x1 . 111)]]

 ;; an optional calling-convention spec at the head of a form: a named symbol,
 ;; or (cc N) for ids without a printer name. Returns the id or #f (not a cc
 ;; spec); errors on a malformed one.
 [define
  (cc-spec x form)
  [cond
    ...
    [let
     ...
     [cond
      [(find (lambda (p) (= (cdr p) n)) call-convs)
       ...
       [lambda
        (p)
        (error "this calling convention has a name; use it" (car p) form)]]
      (else n)]]]
   (else #f)]]

 ;; initializers: literals, undef/zeroinitializer/null, @globals, (c "bytes") /
 ;; (cz "bytes"), and per-element-typed aggregates like ((i64 1) (i32 2)) --
 ;; exactly how IR spells them. elem-resolve (optional): resolver for aggregate
 ;; elements -- the instruction path passes resolve-operand so elements may be
 ;; blockaddress constants, which need the enclosing function's blocks
 [define
  (resolve-constant ctx globals ty form . opt)
  [define
   elem-resolve
   [if
    (pair? opt)
  ...
  [cond
   ...
   [(and (pair? form) (memq (car form) '(c cz)))
    ...
    [unless
     ...
     [error
      "expected (c \"bytes\") or (cz \"bytes\"), a string or a bytevector"
      form]]
   ...
   [(and (pair? form) (eq? (car form) 'blockaddress))
    ...
    [let*
     [[fn
       [or
        (hashtable-ref globals (cadr form) #f)
        (error "unbound function in blockaddress" form)]]
      [tbl
       [or
        (and (function-blocks) (hashtable-ref (function-blocks) (cadr form) #f))
        (error "blockaddress target is not a define" form)]]
      [bb
       [or
        (hashtable-ref tbl (caddr form) #f)
    ...
    [let
     ...
     [cond
      ...
      [else
       [unless
        (= (length rest) 3)
        (error "expected (binop nuw|nsw? type constant constant)" form)]
       [when
        (and (or nuw nsw) (eq? (car form) 'xor))
        (error "xor carries no wrap flags" form)]
       [when
        (and nuw nsw)
        (error "the C API cannot construct nuw+nsw constexprs" form)]
       [let
        ((ety (resolve-type ctx (car rest))))
        [ir:const-binop
         (car form)
         nuw
         nsw
   ...
   [(and (pair? form) (memq (car form) '(extractelement insertelement)))
    ...
    [let
     ((gs (map constant-group (cdr form))))
     [if
      (eq? (car form) 'extractelement)
      [begin
       [unless
        (= (length gs) 2)
   ...
   [(and (pair? form) (eq? (car form) 'splat))
    ...
    [unless
     (and ty (= (length form) 2) (pair? (cadr form)) (= (length (cadr form)) 2))
     ...
     [cond
      ...
      [else
       [unless
        ...
        (error "expected (getelementptr flags? src-type groups...)" form)]
       [let
        ((groups (map constant-group (cdr rest))))
        [ir:const-gep
         (resolve-type ctx (car rest))
         (car groups)
         (cdr groups)
         flags]]]]]]
   [(and (pair? form) (for-all pair? form))
    [let
     ((elts (map constant-group form)))
     [case
      (ir:type-kind ty)
      ((array) (ir:const-array (resolve-type ctx (caar form)) elts))
      ((vector) (ir:const-vector elts))
      ;; identified (named OR anonymous) vs literal struct types -- the name
      ;; alone misses anonymous identified structs
      [(struct)
       [if
        (ir:literal-struct-type? ty)
        (ir:const-struct ctx elts (ir:packed-struct-type? ty))
        (ir:const-named-struct ty elts)]]
      (else (error "aggregate initializer for a non-aggregate type" form))]]]
   (else (error "invalid constant initializer" form))]]

 ;; (= @name (global|constant linkage? type init? attr*)) -- the kind is the
 ;; head, linkage is a modifier after it (like instruction flags); no
 ;; initializer only for external/extern_weak declarations
 [define
  (parse-global item)
  ;; -> (values name linkage-int-or-#f constant? type-form init-form
  ;;            attrs addrspace-or-#f externally-initialized?)
  [unless
   (and (= (length item) 3) (global-name? (cadr item)) (pair? (caddr item)))
   (error "expected (= @name (global|constant ...))" item)]
  [let
   ((name (cadr item)) (rhs (caddr item)))
   [unless
    (and (memq (car rhs) '(global constant)) (pair? (cdr rhs)))
    (error "expected (global ...) or (constant ...)" item)]
   [let*
    [(constant? (eq? (car rhs) 'constant))
     [as
      [let
       ((x (cadr rhs)))
       [and
     ...
     (rest (cddr rhs))
     [attr?
      (lambda (f) (and (pair? f) (memq (car f) '(align visibility section))))]
     (init (and (pair? rest) (not (attr? (car rest))) (car rest)))
     (attrs (if init (cdr rest) rest))]
    [unless
     (for-all attr? attrs)
     (error "malformed global attributes" attrs item)]
    (values name (and lk (cdr lk)) constant? ty-form init attrs as ext-init?)]]]
 ...
 [define
  ...
  [unless
   [and
    (pair? item)
    (memq (car item) '(define declare = type datalayout triple module-asm))]
   [error
    "unknown module item (expected define, declare, type, datalayout, triple, module-asm or (= @name ...))"
    item]]
  (car item)]

 ;; (type %name (struct ...)|(packed-struct ...)|opaque) -- named struct types;
 ;; created before anything resolves types, bodies filled second so structs may
 ...
 [define
  ...
  [unless
   [and
    (= (length item) 3)
    (local-name? (cadr item))
    [or
     (eq? (caddr item) 'opaque)
     [and
      (pair? (caddr item))
      (memq (car (caddr item)) '(struct packed-struct))]]]
   (error "expected (type %name (struct ...)|opaque)" item)]]

 ;; all-digit type names are anonymous (like values): the struct is created
 ;; UNNAMED so the printer numbers it; the binding lives here
 ...
 [define
  (create-type-item! ctx item)
  [when
   (eq? (car item) 'type)
   (check-type-item item)
   [let
    ((nm (strip-sigil (cadr item))))
    (when (lookup-type ctx nm) (error "duplicate named type" (cadr item)))
    [if
     (anonymous-name? nm)
     (hashtable-set! (anon-types) nm (ir:create-named-struct ctx ""))
     (ir:create-named-struct ctx nm)]]]]

 [define
  (fill-type-item! ctx item)
  [when
   (eq? (car item) 'type)
   [let
    ((body (caddr item)))
 ...
 [define
  (item-signature item)
  (unless (>= (length item) 3) (error "malformed module item" item))
  [let*
   [(lk (and (symbol? (cadr item)) (assq (cadr item) linkages)))
   ...
   [let
    ((sig (caddr item)))
    [unless
     (and (pair? sig) (global-name? (car sig)))
 ...
 [define
  (check-param p item)
  [unless
   (and (pair? p) (pair? (cdr p)) (null? (cddr p)) (local-name? (cadr p)))
   (error "parameter must be (type %name)" p item)]]

 ;; pass 1: create every function and global variable up front, so bodies and
 ;; initializers may reference them in any order
 [define
  (alias-item? item)
  [and
   (eq? (item-kind item) '=)
   (pair? (caddr item))
   (eq? (car (caddr item)) 'alias)]]
 ...
 [define
  ...
  [let
   ...
   [if
    [or
     (memq kind '(type datalayout triple module-asm))
     (alias-item? item)
     (ifunc-item? item)]
    (void)                      ; types have their own passes; aliases come
                                ; after
    [if
     (eq? kind '=)
     [let-values
      [[(name lk constant? ty-form init attrs as ext-init?)
        (parse-global item)]]
      [when
       (hashtable-ref globals name #f)
       (error "duplicate global name" name)]
      ;; explicit address space always: LLVMAddGlobal would use the datalayout's
      ;; default-globals space (G) instead of 0
      [hashtable-set!
       globals
       name
       (ir:add-global m (resolve-type ctx ty-form) (llvm-name name) (or as 0))]]
     (declare-function! ctx m globals item kind)]]]]

 ;; an (attributes ...) element: a bare symbol is a valueless enum attribute
 ;; (the names LLVM's own kind lookup accepts, via (sll attributes)); ("key")
 ;; and ("key" "value") are string attributes. Valued enums and type attributes
 ;; are not modeled.
 [define
  (resolve-attribute ctx spec form fname)
  [cond
   [(symbol? spec)
    [let
     ((kind (attrs:enum-name->kind spec)))
     [unless
      kind
      [error
       "unknown enum attribute (valued and type attributes are not modeled)"
       spec
       form
       fname]]
   ...
   [else
    [error
     "attribute must be a symbol, (\"key\") or (\"key\" \"value\")"
     spec
     form
     fname]]]]
 ...
 [define
  (declare-function! ctx m globals item kind)
  [let-values
   (((retty-form fname rest0 lk ccv body) (item-signature item)))
   [let-values
    (((rest variadic?) (split-variadic rest0)))
    [when
     (hashtable-ref globals fname #f)
     (error "duplicate global name" fname)]
    [let*
     [(retty (resolve-type ctx retty-form))
      [ptys
       [case
        kind
        [(define)
         [map
          (lambda (p) (check-param p item) (resolve-type ctx (car p)))
          rest]]
     ...
     [let
      ...
      ;; optional decorations after the signature, in print order: (attributes
      ;; ...) then (align N) then (gc "name")
      [let
       deco
       ((b body))
       [when
        (and (pair? b) (pair? (car b)))
        [case
         (car (car b))
         [(attributes)
          [for-each
           [lambda
            ...
            [ir:add-function-attribute!
             f
             (resolve-attribute ctx spec (car b) fname)]]
           (cdr (car b))]
          (deco (cdr b))]
         [(align)
          [let
           ((a (car b)))
           [unless
            (and (= (length a) 2) (fixnum? (cadr a)) (positive? (cadr a)))
 ...
 [define
  ...
  [when
   [and
    ...
    (not (ifunc-item? item))]
   [let-values
    (((name lk constant? ty-form init attrs as ext-init?) (parse-global item)))
    [let
     ((g (hashtable-ref globals name #f)))
     ...
     [if
      ...
      [unless
       (memq lk '(0 12))
       [error
        "a global without an initializer must be external or extern_weak"
        name]]]
     (apply-attrs! g attrs item)]]]
  [when
   (eq? (item-kind item) 'define)
   [let-values
    (((retty-form fname params lk ccv full-body0) (item-signature item)))
    [let*
     [(f (hashtable-ref globals fname #f))
      ;; (attributes ...)/(align N)/(gc "...") were applied in the declare pass
      [full-body
       [let
        skip
        ((b full-body0))
 ...
 ;; Build an sll program (a list of module items) into a fresh (llvm ir) module
 ;; in the given context. (= @a (alias linkage? value-type (ptr aliasee))). Two
 ;; phases so that (a) aliases print in program order (LLVM prints creation
 ;; order) and (b) aliases may reference aliases in any order: create each with
 ;; a null aliasee first, then patch the aliasees.
 [define
  (alias-parts ctx item)
  [let*
   [(rhs (caddr item))
    (rest (cdr rhs))
    [as
     (let ((x (car rest))) (and (pair? x) (eq? (car x) 'addrspace) (cadr x)))]
    (rest (if as (cdr rest) rest))
    (lk (and (pair? rest) (symbol? (car rest)) (assq (car rest) linkages)))
    (rest (if lk (cdr rest) rest))]
   [unless
    (and (= (length rest) 2) (pair? (cadr rest)) (= (length (cadr rest)) 2))
    [error
     "expected (= @name (alias (addrspace n)? linkage? type (ptr aliasee)))"
     item]]
   (values (cadr item) as lk (car rest) (cadr rest))]]

 ;; (= @i (ifunc linkage? fn-type (ptr @resolver))) -- same two-phase scheme as
 ;; aliases (creation order = print order)
 [define
  (ifunc-parts ctx item)
  [let*
   [(rest (cdr (caddr item)))
    (lk (and (pair? rest) (symbol? (car rest)) (assq (car rest) linkages)))
    (rest (if lk (cdr rest) rest))]
   [unless
    (and (= (length rest) 2) (pair? (cadr rest)) (= (length (cadr rest)) 2))
    (error "expected (= @name (ifunc linkage? fn-type (ptr resolver)))" item)]
   (values (cadr item) lk (car rest) (cadr rest))]]

 [define
  (create-ifunc! ctx m globals item)
  [when
   (ifunc-item? item)
   [let-values
    (((name lk fnty-form g) (ifunc-parts ctx item)))
    (when (hashtable-ref globals name #f) (error "duplicate global name" name))
    [let
     [[i
       [ir:add-ifunc
        m
 ...
 [define
  (patch-ifunc! ctx m globals item)
  [when
   (ifunc-item? item)
   [let-values
    (((name lk fnty-form g) (ifunc-parts ctx item)))
 ...
 [define
  (create-alias! ctx m globals item)
  [when
   (alias-item? item)
   [let-values
    (((name as lk vty-form g) (alias-parts ctx item)))
    (when (hashtable-ref globals name #f) (error "duplicate global name" name))
    [let
     [[a
       [ir:add-alias
        m
        (resolve-type ctx vty-form)
        (ir:const-null (ir:pointer-type ctx (or as 0)))
        (llvm-name name)
        (or as 0)]]]
 ...
 [define
  (patch-alias! ctx m globals item)
  [when
   (alias-item? item)
   [let-values
    (((name as lk vty-form g) (alias-parts ctx item)))
 ...
 [define
  ...
  [let
   ...
   ;; declarations are SATISFIED BY DEFINITIONS: a (declare ...) naming a
   ;; function this module defines, an external global (no initializer) whose
   ;; name a defining global provides, or a repeat of either, is skipped --
   ;; linker semantics, so a unit may declare every extern it uses whether the
   ;; definition sits in this module or in another one of the same image
   ;; (M10/S3). Two definitions of one name still collide.
   [let
    ((defined (make-eq-hashtable)) (declared (make-eq-hashtable)))
    [define
     (global-decl? item)
    ...
    [define
     (redundant? name)
     (or (hashtable-ref defined name #f) (hashtable-ref declared name #f))]
    [for-each
     [lambda
      (item)
      [case
       ...
       [(define)
        [call-with-values
         (lambda () (item-signature item))
         (lambda (retty fname . rest) (hashtable-set! defined fname #t))]]
       [(=)
        [unless
         (or (alias-item? item) (ifunc-item? item))
         [let-values
          (((name def?) (global-decl? item)))
   ...
   [parameterize
    ((function-blocks (make-eq-hashtable)))
 ...
 [define
  (load-sll path)
  [let
   ((env (copy-environment (environment '(chezscheme)) #t)))
      ...
      [let
       ...
       [cond
        ...
        [(and (pair? d) (eq? (car d) 'scheme))
         ...
         (loop acc)]
        ;; each item evaluates AS IT IS READ, so stateful escapes observe strict
        ;; top-to-bottom file order (one quasiquote over the whole file would
        ;; leave the order of ,expr side effects unspecified)
        ...
        (else (loop (cons (eval (list 'quasiquote d) env) acc)))]]]]]]]

 ;; The one-stop shop: compile a program in memory and hand back one of its
 ;; functions as an ordinary Scheme procedure. The procedure keeps the
 ;; underlying JIT alive.
 (define (procedure prog name) (jit:function (jit prog) name))

 ;; Module items that expose the module's .llvm_stackmaps section to run-time
 ;; lookup: codegen's own section symbol is local (invisible to ORC and to
 ;; normal linking), so an exported keeper pointer is the portable handle.
 ;; Splice into any program whose GC stack maps must be readable at run time;
 ;; consume via jit:stackmap-address (JIT) or the sll_stackmaps_keeper symbol
 ...
 [define
  (compile-through prog opts emit)
  (define (opt key) (cond ((memq key opts) => cadr) (else #f)))
  [let*
   [(own-machine? (not (opt 'machine)))
