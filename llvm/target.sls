...
(library (llvm target)
  ...
  (define (host-cpu-name)
    (base:cstring->string/dispose (LLVMGetHostCPUName)))
  ...
  ;; (make-machine)                       -> host defaults, -O2
  ;; (make-machine triple cpu features opt-level)
  (define make-machine
    (case-lambda
      [() (make-machine (default-triple)
  ...
  (define (machine-live-ptr tm)
    (unless (eq? (machine-state tm) 'owned)
      (base:error 'machine "target machine is no longer live"
                  (machine-state tm)))
    (machine-ptr tm))

  (define (machine-dispose! tm)
    (when (eq? (machine-state tm) 'owned)
