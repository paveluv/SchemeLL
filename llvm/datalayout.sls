;;; (llvm datalayout) -- the datalayout string as structured data. The asm-DSL
;;; philosophy applied to LLVM's other little language: model the STRUCTURE the
;;; spec defines, pass unrecognized leaves through verbatim, and demand
;;; byte-identical round-trips (parse then unparse is the identity on every
;;; layout string in LLVM's own test corpus -- the test suite holds it to that).
;;;
...
[library
 ...
 [define
  ...
  [and
   ...
   [let
    ...
    [or
     (= i (string-length s))
     (and (char<=? #\0 (string-ref s i) #\9) (loop (+ i 1)))]]]]

 [define
  (num s comp)                  ; digit string -> exact integer, else #f
  (and (all-digits? s) (string->number s 10))]

 [define
  (nums parts comp)             ; every part a number, else #f
  [let
   loop
   ((ps parts) (acc '()))
   [cond
  ...
  [if
   ...
   [let*
    [[colon
     ...
     [tail-parts
      (if colon (split (substring c (+ colon 1) (string-length c)) #\:) '())]]
    [cond
     ...
     [(and (string=? head "m") (= (length tail-parts) 1))
      [cond
       ((assoc (car tail-parts) manglings) => (lambda (p) `(mangling ,(cdr p))))
       (else (raw))]]
     [(and (string=? head "ni") (pair? tail-parts))
      [cond
       ((nums tail-parts c) => (lambda (ns) `(non-integral ,@ns)))
       (else (raw))]]
     [(and (string=? head "a") (pair? tail-parts))
      [cond
       ((nums tail-parts c) => (lambda (ns) `(aggregate ,@ns)))
       (else (raw))]]
     ((string=? head "") (raw))
     ;; letter+number, no colon parts
     [(and (not colon) (> (string-length c) 1))
      [let
       ((letter (string-ref c 0)) (rest (substring c 1 (string-length c))))
       [case
        letter
        ((#\n) (cond ((num rest c) => (lambda (n) `(native ,n))) (else (raw))))
        [(#\S)
         (cond ((num rest c) => (lambda (n) `(stack-align ,n))) (else (raw)))]
        [(#\P)
         (cond ((num rest c) => (lambda (n) `(program-as ,n))) (else (raw)))]
        [(#\G)
         (cond ((num rest c) => (lambda (n) `(globals-as ,n))) (else (raw)))]
        [(#\A)
         (cond ((num rest c) => (lambda (n) `(alloca-as ,n))) (else (raw)))]
        [(#\F)
         [let
          [(kind (string-ref c 1))
      ...
      [let*
       ...
       [cond
        ...
        [(char=? letter #\n)
         ...
         [cond
          ((num selfnum c) => (lambda (w) `(native ,w ,@ns)))
          (else (raw))]]
        [(char=? letter #\p)
         [cond
          ((string=? selfnum "") `(ptr ,@ns))
          ((num selfnum c) => (lambda (as) `(ptr (addrspace ,as) ,@ns)))
          (else (raw))]]
        [(memv letter '(#\i #\v #\f))
         [cond
          [(num selfnum c)
           ...
           [lambda
            (sz)
            `[,(case letter ((#\i) 'int) ((#\v) 'vector) ((#\f) 'float))
              ,sz
              ,@ns]]]
          (else (raw))]]
        (else (raw))]]]
 ...
 [define
  (join-nums prefix ns)
  (apply string-append prefix (map (lambda (n) (format ":~a" n)) ns))]
 ...
 [define
  ...
  [unless
   [and
    (pair? ns)
    (for-all (lambda (n) (and (integer? n) (exact? n) (>= n 0))) ns)]
 ...
 [define
  ...
  [case
   ...
   [(mangling)
    [cond
     [(find (lambda (p) (eq? (cdr p) (cadr f))) manglings)
      =>
    ...
    [format
     ...
     [case
      ...
      [else
       [base:error
        'dl:unparse
        "fn-ptr-align kind must be independent or multiple"
        f]]]
     (caddr f)]]
   [(ptr)
    [if
     (and (pair? (cadr f)) (eq? (car (cadr f)) 'addrspace))
     [begin
      (check-nums f (cons (cadr (cadr f)) (cddr f)))
      (join-nums (format "p~a" (cadr (cadr f))) (cddr f))]
     (begin (check-nums f (cdr f)) (join-nums "p" (cdr f)))]]
   ((int) (check-nums f (cdr f)) (join-nums (format "i~a" (cadr f)) (cddr f)))
   [(vector)
    (check-nums f (cdr f))
    (join-nums (format "v~a" (cadr f)) (cddr f))]
   ((float) (check-nums f (cdr f)) (join-nums (format "f~a" (cadr f)) (cddr f)))
   ...
   [(non-integral)
    ...
    [when
     ...
     [base:error
      'dl:unparse
      "address space 0 cannot be non-integral (LLVM rejects ni:0)"
      f]]
    (join-nums "ni" (cdr f))]
   [(raw)
    [unless
     (and (string? (cadr f)) (null? (cddr f)))
 ...
 ;; layout string -> list of component forms; unrecognized components come back
 ;; as (raw "...") rather than errors, because LLVM's own parser is the validity
 ;; judge -- this library only refuses what it cannot round-trip (nothing)
 [define
  (parse s)
 ...
 [define
  ...
  [if
   ...
   ""
   [let
    ((cs (map form->component forms)))
    (fold-left (lambda (acc c) (string-append acc "-" c)) (car cs) (cdr cs))]]]]
