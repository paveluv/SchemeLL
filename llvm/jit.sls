;;; (llvm jit) -- layer 2: ORC LLJIT. Compile LLVM modules fully in memory and
;;; expose JIT'd functions as ordinary Scheme procedures.
;;;
...
[library
 ...
 [define-record-type
  (jit-context $make-context context?)
  [fields
   (immutable tsctx context-tsctx)
 ...
 ;; ThreadSafeContext; context-ir gives the (borrowed) (llvm ir) context record
 ;; to pass to ir:make-module.
 [define
  (make-context)
  [let
 ...
 [define
  (context-live-tsctx jc)
  [unless
   (eq? (context-state jc) 'owned)
   (base:error 'jit-context "jit context is no longer live" (context-state jc))]
  (context-tsctx jc)]

 ;; Safe to call once the modules built in this context have been added to a
 ;; JIT: the ThreadSafeContext is refcounted and stays alive underneath. Do NOT
 ;; call while an un-added module still uses the context.
 [define
  (context-dispose! jc)
  [when
   (eq? (context-state jc) 'owned)
 ...
 [define
  (sweep-dead-jits!)
  (let loop () (let ((j (jit-guardian))) (when j (dispose! j) (loop))))]
 ...
 [define
  ...
  [let-values
   [[(err ptr)
     (base:call-with-out-ptr (lambda (out) (LLVMOrcCreateLLJIT out builder)))]]
 ...
 [define
  ...
  [let
   ((ft (ir:function-type-of f)))
   [if
    (ir:type-vararg? ft)
    '(unsupported . "vararg functions are not callable via jit:function")
    [let
     [(ret (llvm-type->foreign-type (ir:type-return-type ft)))
      (args (map llvm-type->foreign-type (ir:type-param-types ft)))]
     [if
      (and ret (for-all values args))
      (cons ret args)
      [cons
       'unsupported
       "parameter or return type not representable in Chez FFI"]]]]]]

 [define
  (capture-signatures! j m)
  [let
   ((sigs (jit-signatures j)))
   [let
    loop
    ((f (LLVMGetFirstFunction (ir:module-live-ptr m))))
    [unless
     (base:null-ptr? f)
     [unless
      (ir:declaration? f)
 ...
 [define
  ...
  [unless
   [eqv?
    (ir:context-live-ptr (ir:module-context m))
    (ir:context-live-ptr (context-ir jc))]
   [base:error
    'jit:add-module!
    "module was not created in this jit context"
    m
    jc]]
  ...
  [let
   ...
   (ir:module-consume! m)
   [let
    ((tsm (LLVMOrcCreateNewThreadSafeModule mod-ptr tsctx)))
    ;; AddLLVMIRModule consumes tsm even on error
    [base:check-error-ref
     'jit:add-module!
     (LLVMOrcLLJITAddLLVMIRModule (jit-live-ptr j) (jit-dylib j) tsm)]]]]

 ;; Raw entry-point address of a JIT'd function (triggers compilation).
 [define
  (lookup-address j name)
  [let-values
   [[(err addr)
     [base:call-with-out-ptr
      (lambda (out) (LLVMOrcLLJITLookup (jit-live-ptr j) out name))]]]
   (base:check-error-ref 'jit:lookup-address err)
   ;; materialization just ran: inline-asm parse errors report success + an
   ;; error-severity diagnostic
   (base:check-diagnostics! 'jit:lookup-address)
   addr]]

 ;; The payoff: a JIT'd function as a ready-to-call Scheme procedure, with the
 ;; foreign signature derived from the function's LLVM type.
 [define
  (function j name)
  [let
   ((sig (hashtable-ref (jit-signatures j) name #f)))
   [cond
    [(not sig)
     (base:error 'jit:function "function not defined in any added module" name)]
    ((eq? (car sig) 'unsupported) (base:error 'jit:function (cdr sig) name))
    [else
     [let*
      [(addr (lookup-address j name))
       [fp
        [eval
 ...
 [define
  ...
  [case-lambda
   ...
   [(j keeper-name)
    (define (try name) (guard (e (#t #f)) (lookup-address j name)))
    [let
     [[keeper-name
       (if (symbol? keeper-name) (symbol->string keeper-name) keeper-name)]]
     [cond
      ((try "__LLVM_StackMaps"))
      ((try keeper-name) => (lambda (keeper) (foreign-ref 'void* keeper 0)))
      [else
       [base:error
        'jit:stackmap-address
