...
(library (llvm base)
  (export null-ptr null-ptr?
          error take-diagnostics! install-diagnostic-handler!
          check-diagnostics!
  ...
  ;; ---- LLVM diagnostics -----------------------------------------------------
  ;; Many codegen problems (inline-asm parse errors above all) are
  ;; reported through a per-context diagnostic callback, not through
  ;; return values. Raising from inside the callback would unwind
  ;; through LLVM's C++ frames, so the handler only RECORDS; the
  ;; project error raiser below drains the record into whatever
  ;; condition is raised next, and take-diagnostics! exposes it.

  (define captured-diagnostics '())   ; (severity . message), newest first

  (define severity-names '#(error warning remark note))

  (define diag-entry-point
    (let ([cb (foreign-callable
                (lambda (di payload)
                  (let* ([cmsg (LLVMGetDiagInfoDescription di)]
                         [msg (or (cstring->string cmsg) "?")]
                         [sev (LLVMGetDiagInfoSeverity di)])
                    (LLVMDisposeMessage cmsg)
                    (set! captured-diagnostics
                      (cons (cons (if (< sev 4)
                                      (vector-ref severity-names sev)
                                      sev)
                                  msg)
  ...
  (define (install-diagnostic-handler! ctx-ptr)
    (LLVMContextSetDiagnosticHandler ctx-ptr diag-entry-point null-ptr))

  ;; drain and return captured diagnostics, oldest first
  (define (take-diagnostics!)
    (let ([d captured-diagnostics])
      (set! captured-diagnostics '())
      (reverse d)))

  ;; Some failures (inline-asm parse errors above all) are ONLY
  ;; visible as captured error-severity diagnostics: LLVM's emission
  ;; entry points still report success. Call after codegen.
  (define (check-diagnostics! who)
    (let* ([diags (take-diagnostics!)]
           [errs (filter (lambda (d) (eq? (car d) 'error)) diags)])
      (unless (null? errs)
        (error who
               (apply string-append "codegen reported errors:"
