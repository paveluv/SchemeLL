;;; (sll render) -- render an sll program to textual LLVM IR, in pure Scheme (no
;;; LLVM involved). The output is not canonical -- it only needs to PARSE;
;;; LLVM's printer canonicalizes both sides of any comparison.
;;;
;;; This is a testing tool (see project/coverage-plan.md): parsing the rendered
;;; text constructs modules through LLVM's parser, which uses the direct
;;; instruction constructors and never constant-folds -- the strict verifier for
;;; programs the C-API builder would fold.
;;;
;;; Caveat: all-digit %N names render explicitly (`%0 =`, `0:`), which the
;;; parser accepts only when the numbering matches its slot counter -- true by
...
[library
 ...
 ;; single-allocation join: the naive string-append fold is quadratic, which
 ;; shows on multi-thousand-instruction modules
 [define
  (join sep xs)
  [if
   (null? xs)
 ...
 ;; a name may appear unquoted when it is an all-digit numeric ID (which MUST
 ;; stay raw -- quoting would make it a different name) or matches LLVM's
 ;; [a-zA-Z$._-][a-zA-Z$._0-9-]* (ASCII only; char-alphabetic? is Unicode-aware
   ...
   [or
    [let
     ...
     [or
      ...
      (and (char<=? #\0 (string-ref s i) #\9) (all-digits (fx+ i 1)))]]
    [and
     (name-char? (string-ref s 0) #f)
     [let
      ok
      ((i 1))
      [or
       (fx= i (string-length s))
       (and (name-char? (string-ref s i) #t) (ok (fx+ i 1)))]]]]]]

 [define
  (escape-bytes bv extra-nul?)
  [let
   ((out (open-output-string)))
   [do
    ((i 0 (fx+ i 1)))
    ...
    [let
     ...
     [if
      (and (fx>= b 32) (fx<= b 126) (not (fx= b 34)) (not (fx= b 92))) ; " and \
 ...
 [define
  (quoted s)                    ; an IR quoted string literal, escaped
  (string-append "\"" (escape-bytes (string->utf8 s) #f) "\"")]

 [define
  (name->text sym)              ; %x / @x, quoted when needed
  [let*
   ((s (symbol->string sym)) (rest (substring s 1 (string-length s))))
   (string-append (substring s 0 1) (if (safe-name? rest) rest (quoted rest)))]]

 [define
  (label-text sym)              ; block label, no sigil
  [let
   ((rest (let ((s (symbol->string sym))) (substring s 1 (string-length s)))))
 ...
 [define
  ...
  [cond
   [(symbol? t)
    [if
     (char=? (string-ref (symbol->string t) 0) #\%)
     (name->text t)
     (symbol->string t)]]
   [(pair? t)
    [case
      ...
      [format
       ...
       [join
        ...
        [cons
         (quoted (cadr t))
         [map
          (lambda (x) (if (integer? x) (number->string x) (type->text x)))
          (cddr t)]]]]]
     [(fn)
      [let-values
       ...
       [format
        ...
        [join
         ", "
         (append (map type->text (cdr parts)) (if variadic? '("...") '()))]]]]
 ...
 ;; fp constants render bit-exactly in IR's hex forms. sll carries them as
 ;; Scheme flonums (unbuild guarantees exact double representability and rejects
 ;; non-double NaNs), so the target-type bits are derivable:
 ;; half/bfloat/float/double use the plain 16-hex double form (the parser
 ;; converts exactly-representable values), x86_fp80 needs 0xK, fp128 0xL,
 ;; ppc_fp128 0xM.
 [define
  (double-bits x)               ; the 64 bits of x, as an exact integer
  [let
   ((bv (make-bytevector 8)))
   (bytevector-ieee-double-set! bv 0 x (endianness big))
   [do
    ((i 0 (fx+ i 1)) (n 0 (+ (* n 256) (bytevector-u8-ref bv i))))
    ((fx= i 8) n)]]]
 ...
 ;; significand of sig-bits and 15 exponent bits (bias 16383); returns (sign
 ;; exponent significand), significand's msb set for finite non-zero values
 [define
  (widen-double x sig-bits)
  [let*
   [(bits (double-bits x))
   ...
   [cond
    [(= expd 2047)
     [if
      (zero? frac)              ; infinity; NaNs cannot reach here (unbuild)
    ...
    [else
     [let*
      [(m (if (zero? expd) frac (bitwise-ior frac (expt 2 52))))
 ...
 [define
  ...
  [case
   tyf
   [(x86_fp80)                  ; 0xK + sign|exp15|explicit-int-bit+frac63 (20
                                ; hex)
    [let-values
     (((sign e msig) (widen-double x 64)))
     (string-append "0xK" (hexn (+ (* sign 32768) e) 4) (hexn msig 16))]]
   [(fp128)                     ; 0xL + sign|exp15|frac112, implicit msb --
                                ; printed and parsed as two 64-bit words, LOW
                                ; word first
    [let-values
     (((sign e msig) (widen-double x 113)))
     [let
      [[bits
        [bitwise-ior
 ...
 ;; the enclosing define's variadicness: a musttail call forwards `...` only
 ;; when BOTH caller and callee are varargs
 ...
 [define
  (named-kind env t)
  [or
   (and (symbol? t) (hashtable-ref env t #f))
   (error "aggregate constant of unknown named type" t)]]

 [define
  (operand->text env tyf v)
  [cond
   [(symbol? v)
    [case
     v
    ...
    [case
     ...
     [(extractelement insertelement)
      ...
      [format
       "~a (~a)"
       (car v)
       (join ", " (map (lambda (g) (group->text env g)) (cdr v)))]]
       ...
       [if
        ...
        [words
         ...
         [format
          ...
          (type->text (car rest))
          (operand->text env (car rest) (caddr rest))]]]]]
     [(getelementptr)
      ...
      [let
       ...
       ((rest (cdr v)) (flags '()))
       [if
        (memq (car rest) '(inbounds nusw nuw))
        (loop (cdr rest) (cons (car rest) flags))
        [words
         "getelementptr"
         (flags-text (reverse flags))
         [format
          ...
          [join
           ...
           [cons
            (type->text (car rest))
            (map (lambda (g) (group->text env g)) (cdr rest))]]]]]]]
     (else (aggregate->text env tyf v))]]
   (else (error "cannot render operand" v))]]

 [define
  (quoted-bytes s nul?)         ; a string (its utf8) or a bytevector (as is)
  [string-append
   "\""
   (escape-bytes (if (bytevector? s) s (string->utf8 s)) nul?)
   "\""]]

 [define
  (group->text env g)           ; (TY V) -> "TY V"
  (format "~a ~a" (type->text (car g)) (operand->text env (car g) (cadr g)))]

 [define
  (aggregate->text env tyf elems)
  [let
   ((body (join ", " (map (lambda (g) (group->text env g)) elems))))
   [cond
    ...
    ((and (pair? tyf) (eq? (car tyf) 'packed-struct)) (format "<{ ~a }>" body))
    [(symbol? tyf)
     [if
      (eq? (named-kind env tyf) 'packed-struct)
      (format "<{ ~a }>" body)
      (format "{ ~a }" body)]]
 ...
 [define
  ...
  '[trunc
    ...
    addrspacecast]]
 [define
  flag-words                    ; leading modifiers an sll instruction may carry
  '[nsw
    nuw
    exact
    disjoint
 ...
 [define
  ...
  [let
   ...
   ((r rest) (flags '()))
   [if
    (and (pair? r) (symbol? (car r)) (memq (car r) flag-words))
    (loop (cdr r) (cons (car r) flags))
    (values (reverse flags) r)]]]

 [define
  (words . parts)               ; join non-empty strings with single spaces
  ...
  [apply
   ...
   [map
    [lambda
     ...
     [cond
      ((symbol? x) (format " ~a" x))
      ((eq? (car x) 'addrspace) (format ", addrspace(~a)" (cadr x)))
      (else (format ", align ~a" (cadr x)))]]
    rest]]]

 [define
  (app->text env tyslot app forward-varargs?)
  ;; callee(args) with the call-site type; a musttail call in a variadic
  ;; function forwards the varargs as a literal `...`
  [let
   ((callee (car app)) (args (cdr app)))
   [format
    "~a ~a(~a)"
    (type->text tyslot)
    [if
     (and (pair? callee) (eq? (car callee) 'asm))
     (asm->text callee)
     ...
     [map
      [lambda
       ...
       [format
        "~s(~a)"
        (cadr b)
        (join ", " (map (lambda (g) (group->text env g)) (cddr b)))]]
      bs]]]]]
 ...
 [define
  ...
  [words
   ...
   (format "~a, ~a" (quoted-bytes (cadr a) #f) (quoted-bytes (caddr a) #f))]]

 ;; atomic before volatile, as the parser demands
 [define
  (memory-flags-text flags)
  [words
   (if (memq 'atomic flags) "atomic" "")
    ...
    [cond
     ...
     [else
      [case
        ...
        [format
         ...
         [join
          ...
          [map
           [lambda
            (c)
            (format "~a, ~a" (group->text env (car c)) (label-ref (cadr c)))]
           (cdddr args)]]]]
       [(indirectbr)
        [format
         "indirectbr ~a, [~a]"
       ...
       ((fneg) (words "fneg" (flags-text flags) (ty) (v 1 (car args))))
       [(icmp fcmp)
        [words
         (symbol->string op)
         (flags-text flags)
       ...
       [(call)
        ...
        [let*
         ...
         [words
          ...
          "call"
          [flags-text
           (filter (lambda (x) (not (memq x '(tail musttail notail)))) flags)]
          (cc-text cc)
           ...
           [string-append
            [app->text
             ...
             [and
              ...
              #t]]
            ;; call-site attributes: inline, after the argument list, before any
            ;; bundles
            [if
             (null? agroups)
             ""
           ...
           [lambda
            ...
            [if
             ...
             [format
              ...
              (car c)
              (type->text (cadr c))
              (operand->text env (cadr c) (caddr c))]]]
           (cdr args)]]]]
       ((resume) (words "resume" (ty) (v 1 (car args))))
       [(catchswitch)
        [format
         "catchswitch within ~a [~a] unwind ~a"
         (operand->text env #f (car args))
         (join ", " (map label-ref (cadr args)))
         (if (eq? (caddr args) 'caller) "to caller" (label-ref (caddr args)))]]
       [(catchpad cleanuppad)
        [format
         "~a within ~a [~a]"
         op
         (operand->text env #f (car args))
         (join ", " (map (lambda (g) (group->text env g)) (cadr args)))]]
       [(catchret)
        [format
         "catchret from ~a to ~a"
         (operand->text env #f (car args))
         (label-ref (cadr args))]]
       [(cleanupret)
        [format
         "cleanupret from ~a unwind ~a"
         (operand->text env #f (car args))
         (if (eq? (cadr args) 'caller) "to caller" (label-ref (cadr args)))]]
       ...
       ((insertelement) (format "insertelement ~a, ~a, ~a" (g 0) (g 1) (g 2)))
       [(shufflevector)
        [let
         [(mask (cdr (caddr args)))
          [scalable?
           [let
            ((t (car (car args))))
            (and (pair? t) (eq? (car t) 'scalable-vector))]]]
         [if
          scalable?
          ;; scalable shuffles admit only splat masks, spelled as a scalable
          ;; zeroinitializer/poison constant
          [format
           "shufflevector ~a, ~a, <vscale x ~a x i32> ~a"
           (g 0)
           (g 1)
           (length mask)
           [cond
            ((for-all (lambda (e) (eqv? e 0)) mask) "zeroinitializer")
            ((for-all (lambda (e) (eq? e 'poison)) mask) "poison")
            (else (error "non-splat scalable shuffle mask" mask))]]
          [format
           "shufflevector ~a, ~a, <~a x i32> <~a>"
           (g 0)
           (g 1)
           (length mask)
           (join ", " (map (lambda (e) (format "i32 ~a" e)) mask))]]]]
       [(extractvalue)
        [format
         "extractvalue ~a, ~a"
         (g 0)
       ...
       [(getelementptr)
        [words
         ...
         [string-append
          ...
          [apply
           string-append
           (map (lambda (g) (format ", ~a" (group->text env g))) (cdr args))]]]]
 ...
 [define
  cc-words                      ; the named calling conventions, as LLVM prints
                                ; them
  '[fastcc
    coldcc
    ghccc
 ...
 ;; an optional leading cc spec: a named symbol or (cc N); returns it or #f. (cc
 ;; N) prints as ccN, the numbered spelling LLVM uses.
 [define
  (cc-spec x)
  [and
 ...
 [define
  ...
  [let*
   [(rest (cdr (caddr item)))
    (lk (and (symbol? (car rest)) (memq (car rest) linkage-words) (car rest)))
 ...
 [define
  ...
  [let*
   [(name (cadr item))
    ...
    (kind (car rhs))
    (rest (cdr rhs))
    (as (and (pair? (car rest)) (eq? (caar rest) 'addrspace) (cadr (car rest))))
    (rest (if as (cdr rest) rest))
    (lk (and (symbol? (car rest)) (memq (car rest) linkage-words) (car rest)))
    (rest (if lk (cdr rest) rest))
 ...
 [define
  ...
  [let-values
   (((parts variadic?) (split-variadic (cdr sig))))
   [format
    "~a(~a)"
    (name->text (car sig))
    [join
     ", "
     [append
      (map (lambda (p) (if groups? (group->text env p) (type->text p))) parts)
      (if variadic? '("...") '())]]]]]
  ...
  [let*
   ...
   [if
    ...
    [words
     ...
     (cc-text cc)
     (type->text ty)
     (signature->text env sig #f) ; bare types
     [let
      deco
      ((b body) (acc '()))
      [if
    ...
    [let*
     [[attrs
      ...
      [gc
       (and (pair? body) (pair? (car body)) (eq? (caar body) 'gc) (car body))]
      ...
      (variadic? (let-values (((parts v?) (split-variadic (cdr sig)))) v?))]
     [string-append
      [words
       "define"
       (if lk (symbol->string lk) "")
       ...
       [if
        ...
        [format
         "personality ~a ~a"
         (type->text (cadr pers))
         (operand->text env (cadr pers) (caddr pers))]
        ""]
       "{"]
      "\n"
      [parameterize
       ((caller-variadic? variadic?))
       [join
        "\n"
        [map
         [lambda
          ...
          [string-append
           (label-text (cadr b))
           ":\n"
           (join "\n" (map (lambda (i) (insn->text env i)) (cddr b)))]]
         blocks]]]
      "\n}"]]]]]
 ...
 [define
  ...
  [format
   "~a = type ~a"
   (name->text (cadr item))
   (if (eq? (caddr item) 'opaque) "opaque" (type->text (caddr item)))]]
    ...
    [lambda
     ...
     [when
      ...
      [hashtable-set!
       env
       (cadr item)
       (if (pair? (caddr item)) (car (caddr item)) 'opaque)]]]
    prog]
   [string-append
    [join
     "\n"
     [map
      [lambda
       ...
       [case
        (car item)
        ((datalayout) (format "target datalayout = ~s" (cadr item)))
        ((triple) (format "target triple = ~s" (cadr item)))
        [(module-asm)
         ;; one `module asm` directive per stored line
         [let
          ((p (open-string-input-port (cadr item))) (out '()))
          [let
           loop
           ()
           [let
            ...
            [if
             ...
             [join
              "\n"
              [map
               (lambda (x) (string-append "module asm " (quoted x)))
               (reverse out)]]
             (begin (set! out (cons l out)) (loop))]]]]]
