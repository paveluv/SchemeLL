...
(import (chezscheme) (prefix (tests harness) t:) (prefix (llvm ir) ir:))
...
[ir:build-ret
 ...
 [ir:build-add
  b
  (ir:function-param add-fn 0)
  (ir:function-param add-fn 1)
  "sum"]]

(t:check "verify-module passes on valid IR" (begin (ir:verify-module mod) #t))

[t:check
 "module->string prints the function"
 [let
  ((ir-text (ir:module->string mod)))
  [and
   (string? ir-text)
   [let
    ((n (string-length ir-text)))
    [let
     loop
     ((i 0))                    ; contains "define"?
     [cond
...
[t:check
 "type-kind of function type"
 (eq? (ir:type-kind add-fn-type) 'function)]

(t:check "return type round-trips" (eqv? (ir:type-return-type add-fn-type) i32))

[t:check
 "param types round-trip"
 (equal? (ir:type-param-types add-fn-type) (list i32 i32))]

[t:check
 "function-type-of recovers the fn type (opaque ptrs)"
...
[t:check
 "named-function returns #f for missing"
 (not (ir:named-function mod "nope"))]
...
(t:check-exn "using a disposed builder raises" (ir:build-ret-void b))

(ir:module-dispose! mod)

(t:check-exn "using a disposed module raises" (ir:module->string mod))

(t:check "double dispose is a no-op" (begin (ir:module-dispose! mod) #t))

(ir:context-dispose! ctx)

(t:check-exn "using a disposed context raises" (ir:int32-type ctx))
