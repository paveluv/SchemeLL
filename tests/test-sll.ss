  ...
  '((define i64 (@leaf (i64 %x))
    ...
    (define i64 (@via-tail (i64 %x))
      (label %entry
        ...
        (ret i64 %r)))))
(t:check "tail call executes" (= ((jit:function (sll:jit tail-prog) "via-tail") 7) 21))
(t:check "tail marker prints" (contains? (sll:dump tail-prog) "tail call i64 @leaf"))
...
(t:check "alloca count prints" (contains? (sll:dump count-prog) "alloca i64, i64 4"))
...
(t:check "string constant readable"
         (= ((jit:function (sll:jit msg-prog) "first-byte"))
            (char->integer #\h)))
...
(t:check-exn "duplicate global name"
             (sll:dump '((= @x (global i64 0))
                         (= @x (global i64 1)))))

(t:section "sll: declare -- cross-module calls in one jit")

(define jc (jit:make-context))
(define m1 (sll:build (jit:context-ir jc) "m1"
                      '((define i64 (@inc (i64 %x))
                          (label %entry
                            (= %r (add i64 %x 1))
                            (ret i64 %r))))))
(define m2 (sll:build (jit:context-ir jc) "m2"
                      '((declare i64 (@inc i64))
