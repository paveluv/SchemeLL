...
(define (mask l)
  (let ([out (open-output-string)] [n (string-length l)])
    (let loop ([i 0])
      (if (>= i n)
          ...
          (let ([c (string-ref l i)])
            (cond
              [(or (char=? c #\%) (char=? c #\@))
               (put-char out c) (put-char out #\V)
               (let skip ([j (+ i 1)])
                 (if (and (< j n)
                          (let ([d (string-ref l j)])
                            (or (char-alphabetic? d) (char-numeric? d)
                                (memv d '(#\. #\_ #\$ #\-)))))
                     (skip (+ j 1))
                     (loop j)))]
              [(char-numeric? c)
...
(define (first-diff a b)
  (let ([pa (open-string-input-port a)] [pb (open-string-input-port b)])
    (let loop ()
      (let ([la (get-line pa)] [lb (get-line pb)])
        (cond
...
(define (probe path)
  (guard (e [#t (printf "PROBE-ERROR ~a~%"
                        (if (message-condition? e) (condition-message e) e))])
    (let* ([ctx (ir:make-context)] [rctx (ir:make-context)]
      ...
      (let* ([a (n:comparable-ir (ir:module->string m))]
        ...
        (let ([b (n:comparable-ir (ir:module->string m2))])
          (cond
            ...
            [(first-diff a b) =>
             (lambda (d)
               (printf "A: ~a~%" (mask (car d))))]))))))

(call-with-input-file failures-file
  (lambda (p)
    (let loop ()
      (let ([l (get-line p)])
        (unless (eof-object? l)
          (let ([sp (let find ([j 0])
                      (cond
                        ...
                        [else (find (+ j 1))]))])
            (when (and sp (after-marker l "MISMATCH"))
              (probe (substring l 0 sp))))
          (loop))))))
