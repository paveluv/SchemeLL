...
(let* ([entry (ir:append-block ctx fact-fn "entry")]
  ...
  (let ([n-1 (ir:build-sub b n (ir:const-int i64 1))])
    (ir:build-ret b (ir:build-mul b n (ir:build-call b fact-type fact-fn (list n-1))))))

;; hypot2(x, y) = x*x + y*y  -- doubles
(define hypot2-fn (ir:add-function mod "hypot2" (ir:function-type f64 (list f64 f64))))
(let ([x (ir:function-param hypot2-fn 0)]
      [y (ir:function-param hypot2-fn 1)])
...
(t:check "module is consumed after jit:add-module!"
...
(t:check "jit:lookup-address returns a nonzero address"
         (positive? (jit:lookup-address j "add")))
(t:check-exn "jit:function raises for unknown name"
...
(define add-decl (ir:add-function mod2 "add" (ir:function-type i32 (list i32 i32))))
...
(ir:build-ret b2 (ir:build-call b2 (ir:function-type i32 (list i32 i32)) add-decl
                                (list (ir:function-param add3-fn 0)
                                      (ir:const-int i32 3))))
(ir:verify-module mod2)
(ir:builder-dispose! b2)
(jit:add-module! j jc mod2)

(t:check "cross-module call: (add3 39) = 42"
         (= ((jit:function j "add3") 39) 42))
(t:check "declarations don't shadow signatures"
         (= (add 1 1) 2))
