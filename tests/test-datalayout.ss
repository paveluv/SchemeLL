...
;;; Round-trip fidelity was developed against every distinct layout
;;; string in LLVM's test corpus (421 strings, all byte-identical);
;;; the curated set here pins one of each component kind, the legacy
;;; raw passthroughs, and the malformed-input crash-resistance.
(import (chezscheme)
...
(for-each
  (lambda (s)
    (t:check (format "round-trip ~a" s)
             (string=? (dl:unparse (dl:parse s)) s)))
...
(t:check "parse produces the documented forms"
         (equal? (dl:parse "e-m:e-p270:32:32-i64:64-n8:16-S128-ni:1-A5-Fi64")
                 '((endian little) (mangling elf)
                   (ptr (addrspace 270) 32 32) (int 64 64)
                   (native 8 16) (stack-align 128)
                   (non-integral 1) (alloca-as 5)
                   (fn-ptr-align independent 64))))

(t:check "explicit p0 stays distinct from bare p"
...
(t:check "bare p has none"
         (equal? (dl:parse "p:32:32") '((ptr 32 32))))

(t:check "construction from forms"
         (string=? (dl:unparse '((endian little) (mangling elf)
                                 (int 64 64) (native 8 16 32 64)
                                 (non-integral 1)))
                   "e-m:e-i64:64-n8:16:32:64-ni:1"))

(t:check "malformed inputs parse as raw, never crash"
         (equal? (map car (dl:parse "^-:32-m.-a:"))
                 '(raw raw raw raw)))
...
(let ([tm (target:make-machine)])
  (let* ([ctx (ir:make-context)]
    ...
    (target:configure-module! m tm '(1))
    (let* ([final (ir:module->string m)]
           [count (let loop ([i 0] [n 0])
                    (if (> (+ i 3) (string-length final))
                        n
                        (loop (+ i 1)
                              (if (string=? (substring final i (+ i 3)) "-ni")
                                  (+ n 1) n))))])
