;;; (tests oracle) -- extract enums from the installed LLVM C headers. The
;;; headers are the machine-readable ground truth the coverage tests check
...
[library
 ...
 [define
  (str-index s sub start)       ; index of sub in s at/after start, or #f
  [let
 ...
 [define
  (last-index-before s sub limit) ; last occurrence strictly before limit
  [let
   loop
   ((i 0) (found #f))
   [let
    ((j (str-index s sub i)))
    (if (or (not j) (>= j limit)) found (loop (+ j 1) j))]]]

 ;; remove /* ... */ and // ... comments
 [define
  (strip-comments s)
  [let
   ((n (string-length s)) (out (open-output-string)))
   [let
    loop
    ((i 0))
    [cond
     ...
     [[and
      ...
      (let ((end (str-index s "*/" (+ i 2)))) (loop (if end (+ end 2) n)))]
     [[and
       (< (+ i 1) n)
       (char=? (string-ref s i) #\/)
       (char=? (string-ref s (+ i 1)) #\/)]
      (let ((end (str-index s "\n" (+ i 2)))) (loop (if end (+ end 1) n)))]
    ...
    [[a
     ...
     [b
      [let
       ...
       ((i n))
       [if
        (and (> i 0) (char-whitespace? (string-ref s (- i 1))))
        (loop (- i 1))
        i]]]]
 ...
 [define
  ...
  [let
   loop
   ((items (split-char body #\,)) (prev -1) (acc '()))
   [if
    (null? items)
    (reverse acc)
    [let
       ...
       [(parts (split-char item #\=))
        ...
        [val
         [if
          ...
          (+ prev 1)
          [let
           ((v (string->number (trim (cadr parts)))))
           (unless v (error 'enum-alist "unparsable enum value" item))
           v]]]]
       (loop (cdr items) val (cons (cons (string->symbol name) val) acc))]]]]]]

 ;; ((name . 2^n) ...) for every `<prefix><name> = (1 << n)` line in the header
 ;; -- for anonymous bitmask enums like LLVMFastMathFlags and
 ;; LLVMGEPNoWrapFlags, whose non-bit entries (None/All) are skipped.
 [define
  (bitmask-alist header prefix)
  [let
   ...
   [let
    ...
    ((i 0) (acc '()))
    [let
     ((j (str-index text prefix i)))
     [if
      (not j)
      (reverse acc)
      [let*
       [(eol (or (str-index text "\n" j) (string-length text)))
        (entry (parse-bit-line (substring text j eol)))]
       (loop eol (if entry (cons entry acc) acc))]]]]]]

 [define
  (parse-bit-line line)         ; "NAME = (1 << N)..." -> (NAME . 2^N) | #f
  [let
   ((eq (str-index line " = (1 << " 0)))
   [and
    eq
    [let*
     [(rest (substring line (+ eq 9) (string-length line)))
      (close (str-index rest ")" 0))]
     [and
      close
      [let
       ((n (string->number (trim (substring rest 0 close)))))
       [and
        (fixnum? n)
        [cons
