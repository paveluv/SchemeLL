;;; A complete main.o that calls libc printf: everything up to the
;;; final link happens through the LLVM API. (Linking against libc is
;;; the system linker's job; the flagship zero-tool executable is
;;; hello.sll + sllc --exe, which avoids libc entirely.)
(import (chezscheme)
        (prefix (sll) sll:)
        (prefix (llvm ir) ir:)
...
(define prog
  '((= @fmt (constant (array 20 i8) (cz "6 * 7 = %ld from .o")))
    ...
    (define i32 (@main)
      (label %entry
        (= %r (call (fn i32 ptr variadic)
                    (@printf (ptr @fmt) (i64 42))))
        (ret i32 0)))))
