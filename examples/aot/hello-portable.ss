...
;;; registers, and numbers all differ per kernel/ISA) is generated by an
;;; ordinary Scheme function.
;;;
;;; This script cross-compiles the SAME source to objects for both Linux ISAs --
;;; the AArch64 one from an x86 host and vice versa. Run with: scheme --libdirs
;;; . --script examples/aot/hello-portable.ss
[import
 (chezscheme)
 (prefix (sll) sll:)
 (prefix (llvm ir) ir:)
...
[define
 (syscall-write+exit abi buf len)
 [case
  abi
  [(linux-x86_64)
   `[[call
     ...
     [call
      i64
      [(asm "syscall" "={rax},{rax},{rdi},~{rcx},~{r11}" sideeffect)
       (i64 231)
       (i64 0)]]]]
  [(linux-aarch64)
   `[[call
      i64
      [(asm "svc #0" "={x0},{x8},{x0},{x1},{x2},~{memory}" sideeffect)
       (i64 64)
       (i64 1)
       (ptr ,buf)
       (i64 ,len)]]
     (call i64 ((asm "svc #0" "={x0},{x8},{x0}" sideeffect) (i64 94) (i64 0)))]]
...
[for-each
 [lambda
  (t)
  [let
   ((abi (car t)) (triple (cadr t)) (backend (caddr t)) (out (cadddr t)))
   (printf "~%=== ~a: generated syscall tail ===~%" abi)
   (for-each (lambda (f) (printf "  ~s~%" f)) (syscall-write+exit abi '%buf 17))
   [if
    (target:initialize-target! backend)
    [let*
     [(ctx (ir:make-context))
