...
;;; forms per target: instruction, register names, constraint punctuation, and
;;; syscall numbers all live inside opaque strings. With the structured DSL the
;;; difference shrinks to a TABLE -- registers as symbols, numbers as numbers --
;;; and one generator turns table rows into correct-by-construction asm forms:
;;; the constraint strings and operand numbering are computed, never spelled.
;;;
...
(define (abi-ref abi key) (cdr (assq key (cdr (assq abi abis)))))

;; one generator covers every ABI: a syscall asm callee taking the number plus
;; NARGS arguments
[define
 (syscall-asm abi nargs)
 [asm:expr
  [list
   `(out (reg ,(car (abi-ref abi 'ret-reg))))
   `(in (reg ,(car (abi-ref abi 'nr-reg))))
   (map (lambda (r) `(in (reg ,r))) (list-head (abi-ref abi 'arg-regs) nargs))
    ...
    [label
     ...
     [call
      ...
      [,(syscall-asm abi 3)
       ...
       (ptr %buf)
       (i64 17)]]
     (call i64 (,(syscall-asm abi 0) (i64 ,(car (abi-ref abi 'sys-exit)))))
     (unreachable)]]]]
 ...
 [lambda
  ...
  [let*
   [(abi (car entry))
    ...
    (out (string-append "/tmp/hello-dsl-" (symbol->string abi) ".o"))]
   [printf
    "~%=== ~a: the generated write syscall ===~%  ~s~%"
    abi
    (syscall-asm abi 3)]
   [if
    (target:initialize-target! (car (abi-ref abi 'backend)))
    [let*
     [(ctx (ir:make-context))
     ...
     (printf "  -> ~a~%" out)]
    [printf
     "  (no ~a backend in this libLLVM; skipped)~%"
