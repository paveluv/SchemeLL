;;; A library object with global data; the triple and datalayout are
;;; ordinary sll items, so the .o is reproducible bit-for-bit.
(import (chezscheme)
        (prefix (sll) sll:)
        (prefix (llvm ir) ir:)
...
(define prog
  '((= @greeting (constant (array 6 i8) (cz "hello")))
    ...
    (define ptr (@get_greeting)
      (label %entry (ret ptr @greeting)))
    (define i32 (@get_version)
      (label %entry
        (= %v (load i32 (ptr @version)))
        (ret i32 %v)))))
