...
(define horizontal-max
  (sll:procedure
    '((define i32 (@hmax4 (i32 %a) (i32 %b) (i32 %c) (i32 %d))
        (label %entry
          ...
          (= %sh (shufflevector ((vector 4 i32) %v) ((vector 4 i32) poison)
                                (mask 2 3 0 1)))
          (= %gt (icmp sgt (vector 4 i32) %v %sh))
          (= %m1 (select ((vector 4 i1) %gt) ((vector 4 i32) %v)
                         ((vector 4 i32) %sh)))
          ;; then by 1
          (= %sh2 (shufflevector ((vector 4 i32) %m1) ((vector 4 i32) poison)
                                 (mask 1 0 3 2)))
          (= %gt2 (icmp sgt (vector 4 i32) %m1 %sh2))
          (= %m2 (select ((vector 4 i1) %gt2) ((vector 4 i32) %m1)
                         ((vector 4 i32) %sh2)))
